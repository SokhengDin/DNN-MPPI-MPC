...
class DifferentialDrive:
    ...
    def forward_kinematic(self, x: cp.array, u: cp.array):
    ...
    def get_state(self) -> cp.ndarray:
        return cp.array([self.x0, self.y0, self.yaw0])
    
    def update_state(self, sampling_rate: float, x: cp.array, u: cp.array) -> cp.array:
...
class MPPIAlgorithms:
    ...
    def __init__(
            ...
            param_alpha: float,
            sigma: cp.ndarray,
            stage_cost_weight: cp.ndarray,
            terminal_cost_weight: cp.ndarray,
        ...
        # mppi variables 
        self.u_prev = cp.zeros((self.T, self.dim_u))

        # ref path info
        self.prev_way_point_idx = 0

    def _calc_input_control(self, observed_x: cp.ndarray) -> Tuple[float, cp.ndarray]:
        ...
        v = cp.zeros((self.K, self.T, self.dim_u))
        ...
        for k in range(self.K):
            ...
            for t in range(1, self.T+1):
                ...
                # add stage cost
                S[k] = self._compute_cost(x) + self.param_gamma * u[t-1].T @ cp.linalg.inv(self.Sigma)@v[k, t-1]
            # Add terminal cost
        ...
        w_epsilon = cp.zeros((self.T, self.dim_u))
        ...
        optimal_traj = cp.zeros((self.T, self.dim_x))
        ...
        sampled_traj_list = cp.zeros((self.K, self.T, self.dim_x))
    ...
    def _compute_weight(self, S: cp.ndarray) -> cp.ndarray:
        ...
        eta = 0.0
        for k in range(self.K):
            eta += cp.exp(-(1.0/self.param_exploration)*(S[k]-rho))
        # Calculate weights
        for k in range(self.K):
            w[k] = (1/eta)*cp.exp(-(1.0/self.param_exploration)*(S[k]-rho))

        return w

    def _state_transition(self, x_t: cp.ndarray, v_t: cp.ndarray) -> cp.ndarray:
        ...
        x = x + speed*cp.cos(yaw) * dt
        y = y + speed*cp.sin(yaw) * dt
        yaw = yaw + omega * dt

        return cp.array([x, y, yaw])
    ...
    def _compute_cost(self, x_t: cp.ndarray) -> float:
    ...
    def _terminal_cost(self, x_t: cp.ndarray) -> float:
    ...
    def _calc_epsilon(self, sigma: cp.ndarray, size_sample: int, size_time_step: int, size_dim_u: int) -> cp.ndarray:
        """Sample epsilon"""
        ...
        # sample epsilon
        mu = cp.zeros((size_dim_u))
        epsilon = cp.random.multivariate_normal(mu, sigma, (size_sample, size_time_step))
        return epsilon
    
    def _g(self, v: cp.ndarray) -> cp.ndarray:
        """Clamping the input control"""
        v[0] = cp.clip(v[0], -self.max_speed, self.max_speed)
        v[1] = cp.clip(v[1], -self.max_omega, self.max_omega)
        return v
    

def plot_trajectories(diff_frame, diff_drive, mppi, delta_t, ref_path, tSim):
    fig, ax = plt.suybplots()
    ax.set_xlabel('X')
    ax.set_ylabel('Y')
    ...
    # Intialize empty list of artists
    robot_artists = []
    optimal_traj_artist, = ax.plot([], [], color='#990099', linestyle="solid", linewidth=1, zorder=5)
    ...
    def animate(i):
        ...
        # Update the sampled trajectory artist
        ...
        if sampled_traj_list.any():
            ...
            for idx, sampled_traj in enumerate(sampled_traj_list):
                ...
                # Extend the sampled trajectory artist
                extended_sampled_traj = cp.zeros((sampled_traj.shape[0]+1, sampled_traj.shape[1]))
...
def generate_circle_points(center_x, center_y, radius, num_points=20):
    angles = cp.linspace(0, 2 * pi, num_points)
    x = center_x + radius * cp.cos(angles)
    y = center_y + radius * cp.sin(angles)
    return x, y


if __name__ == "__main__":
    # Initialize the differential drive model
    init_x = cp.array([0.0, 0.0, 0.0])
    tSim = 500
    ...
    param_lambda = 1.0 
    param_alpha = 0.1
    sigma = cp.array([[0.1, 0.0], [0.0, 0.1]])
    stage_cost_weight = cp.array([5.0, 5.0, 10.0])
    terminal_cost_weight = cp.array([5.0, 5.0, 10.0])
    ...
    ref_path = cp.array([cx, cy, cyaw]).T
