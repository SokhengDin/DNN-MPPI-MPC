...
class DifferentialSimulation:
    ...
    def rotation_angle(self, theta):
        matrix_transform = cp.array([[cp.cos(theta), -cp.sin(theta)], [cp.sin(theta), cp.cos(theta)]]).T
        return matrix_transform

    def robot_shape(self):
        matrix_shape = cp.array([
    ...
    def generate_each_wheel_and_draw(self, x, y, yaw):
        ...
        pos_wheel1 = cp.dot(pos_wheel1.T, self.rotation_angle(yaw)).T
        pos_wheel2 = cp.dot(pos_wheel2.T, self.rotation_angle(yaw)).T
        pos_wheel3 = cp.dot(pos_wheel3.T, self.rotation_angle(yaw)).T
        pos_wheel4 = cp.dot(pos_wheel4.T, self.rotation_angle(yaw)).T

        # Matrix Transforms robot shape
        robot_shape = self.robot_shape()
        robot_shaped = cp.dot(robot_shape.T, self.rotation_angle(yaw)).T

        artists = []
        artists.append(plt.Line2D(cp.asnumpy(robot_shaped[0, :]+x), cp.asnumpy(robot_shaped[1, :]+y), color="black"))
        artists.append(plt.Line2D([x], [y], marker="x", color="red"))
        artists.append(plt.Line2D(cp.asnumpy(pos_wheel1[0, :]+x), cp.asnumpy(pos_wheel1[1, :]+y), color="black"))
        artists.append(plt.Line2D(cp.asnumpy(pos_wheel2[0, :]+x), cp.asnumpy(pos_wheel2[1, :]+y), color="black"))
        artists.append(plt.Line2D(cp.asnumpy(pos_wheel3[0, :]+x), cp.asnumpy(pos_wheel3[1, :]+y), color="black"))
        artists.append(plt.Line2D(cp.asnumpy(pos_wheel4[0, :]+x), cp.asnumpy(pos_wheel4[1, :]+y), color="black"))
