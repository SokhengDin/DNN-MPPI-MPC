...
class MPPIRacecarController:
    def __init__(
            ...
            obstacle_circles: np.ndarray = np.array([[5.0, 5.0, 1.0], [7.0, 7.0, 1.0]]),
            collision_safety_margin_rat: float = 1.5,
        ...
        self.collision_safety_margin_rate = collision_safety_margin_rat
    ...
    def _is_collided(self, x_t: np.ndarray) -> bool:
        ...
        satefy_margin_rate = self.collision_safety_margin_rate
        vw, vl = vw*satefy_margin_rate, vl*satefy_margin_rate
    ...
    def generate_lemniscate_trajectory(self, num_points: int, radius: float) -> np.ndarray:
        t = np.linspace(0, 2 * np.pi, num_points, dtype=np.float32)
        a = radius  # scaling factor for the lemniscate

        x = a * np.cos(t) / (1 + np.sin(t)**2)
        y = a * np.sin(t) * np.cos(t) / (1 + np.sin(t)**2)
        yaw = np.arctan2(np.gradient(y), np.gradient(x))  # calculate yaw as the arctangent of the gradient of y and x
        v = np.ones_like(t) * 5.0  # constant velocity
...
if __name__ == "__main__":
    ...
    num_points = 100
    radius = 10.0
    ref_path = controller.generate_lemniscate_trajectory(num_points, radius)
    ...
    controller.plot_control_signals(optimal_input[:, 0])