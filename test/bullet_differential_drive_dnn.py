...
def plot_control_input(us, output_path):
    # Extract the control inputs
    v_values = [u[0] for u in us]
    omega_values = [u[1] for u in us]

    # Create an iteration array
    iterations = range(len(us))

    # Create a new figure and axis
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(8, 6))

    # Plot linear velocity
    ax1.plot(iterations, v_values, linewidth=2)
    ...
    ax2.plot(iterations, omega_values, linewidth=2)
...
def animate(i, xs, us, simX_history, cube_ids, ax, differential_robot, cube_size, yref_N, safe_distance):
    ...
    safety_circle = plt.Circle((xs[i][0], xs[i][1]), safe_distance, edgecolor='k', linestyle='--', facecolor='none')
...
def run():
    ...
    ani.save('diff_mpc_dnn.mp4', writer='ffmpeg', fps=30)

    plot_control_input(us, 'diff_mpc_dnn')
