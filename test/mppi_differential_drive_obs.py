...
class MPPIAlgorithms:
    ...
    def _compute_cost(self, x_t: np.ndarray) -> float:
        ...
        # Calculate stage cost
        dx = x - self.goal_point[0]
        dy = y - self.goal_point[1]
        ...
        stage_cost = self.stage_cost_weight[0] * distance_to_goal**2 + \
                     self.stage_cost_weight[1] * angle_diff**2
    ...
    def _terminal_cost(self, x_t: np.ndarray) -> float:
        ...
        terminal_cost = self.terminal_cost_weight[0] * distance_to_goal**2 + \
                        self.terminal_cost_weight[1] * angle_diff**2
...
def plot_trajectories(diff_frame, diff_drive, mppi, delta_t, goal_point, tSim):
    fig, ax = plt.subplots()
    ax.set_xlabel('X')
    ax.set_ylabel('Y')
    ...
    def animate(i):
        ...
        # Draw the goal point
        if goal_artist is None:
            goal_artist = plt.Circle(goal_point, 0.2, color='green', zorder=7)
        ...
        return robot_artists + [optimal_traj_artist] + sampled_traj_artists + [goal_artist]

    ani = FuncAnimation(fig, animate, frames=tSim, interval=50, blit=True, repeat=False)
    ani.save("mppi_differential_drive_goal.mp4", writer='ffmpeg', fps=30)
...
def generate_point_trajectory(start_point, end_point, num_points=100):
    x = np.linspace(start_point[0], end_point[0], num_points)
    y = np.linspace(start_point[1], end_point[1], num_points)
    yaw = np.arctan2(end_point[1] - start_point[1], end_point[0] - start_point[0]) * np.ones(num_points)
    return x, y, yaw
...
if __name__ == "__main__":
    ...
    stage_cost_weight = 10*np.array([5.0, 9.0])
    terminal_cost_weight = 10*np.array([5.0, 9.0])
    ...
    plot_trajectories(diff_frame, diff_drive, mppi, delta_t, goal_point, tSim)